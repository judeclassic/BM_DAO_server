...
const ERROR_MODERATOR_SERVICE_NOT_FOUND = {
...
class ModeratorUserServiceService {
    constructor({ authRepo, userModel, userServiceModel, transactionModel }) {
        this.subscribeForAService = ({ accountType, userId, }) => __awaiter(this, void 0, void 0, function* () {
            ...
            const userServiceExists = yield this._userServiceModel.checkIfExist({ userId, accountType: enums_1.ServiceAccountTypeEnum.moderators });
            if (userServiceExists.data)
                return { errors: [ERROR_ALREADY_HAVE_THIS_ACCOUNT] };
            user.data.referal.isGiven = true;
            const isWithdrawed = user.data.updateUserWithdrawableBalance({ amount: user_dto_1.AmountEnum.moderatorSubscriptionPackage1, type: 'charged' });
            if (!isWithdrawed)
                return { errors: [ERROR_NOT_ENOUGH_BALANCE] };
            const userServiceRequest = {
                name: user.data.name,
                work_timeout: Date.parse((new Date()).toISOString()),
                accountType: enums_1.ServiceAccountTypeEnum.moderators,
                userId: userId,
                updatedAt: new Date(),
                createdAt: new Date(),
        ...
        this.getUserService = (userId) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (userServices.error || !userServices.data)
                return { errors: [ERROR_MODERATOR_SERVICE_NOT_FOUND] };
